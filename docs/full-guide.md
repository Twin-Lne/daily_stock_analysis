...
> 本节是 Issue #1497 的文档边界澄清（仅作为参考说明，不构成运行时交付），不包含 Actions 触发机制改造。该问题的运行时交付仍未完成，需在单独实现 PR 中完成。
...
> 该说明对应 Issue #1497 的文档边界澄清，不应理解为该 issue 的 runtime 实现已关闭；相关能力仍需独立实现与验收。
